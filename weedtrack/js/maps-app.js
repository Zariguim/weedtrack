...
// ===================================================================================
//    COLE AQUI AS INFORMAÇÕES DO SEU PROJETO SUPABASE
// ===================================================================================
const SUPABASE_URL = 'https://qtdyakpmgpxweilvszvm.supabase.co'; // <-- COLE A URL AQUI
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF0ZHlha3BtZ3B4d2VpbHZzenZtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU5MTI2NTksImV4cCI6MjA3MTQ4ODY1OX0.gZXvuwV62xnfzj88uqRklP2FJV61JRjCgBZcB1Ri4Vw'; // <-- COLE A CHAVE AQUI

// CORRIGIDO: A inicialização do cliente Supabase foi corrigida.
...
const MapasApp = {
    dom: {
        ...
        kmlNameInput: document.getElementById('kmlNameInput'),
        uploadKmlBtn: document.getElementById('uploadKmlBtn'),
    ...
    bindEvents() {
        this.dom.uploadKmlBtn.addEventListener('click', this.handleKmlUpload.bind(this));
    ...
    async loadKmlDatabase() {
        this.setLoading(true, false, "A carregar talhões do banco de dados...");
        
        // CORRIGIDO: Usa o 'supabaseClient' para a consulta
        ...
        if (error) {
            ...
            this.showToast('Não foi possível carregar os talhões.', 'error');
    ...
    handleKmlSelect(event) {
        ...
        if (talhao && talhao.conteudo_kml) {
            ...
            this.processKml(url);
            this.dom.kmlInput.value = "";
        } else {
            this.showToast('Conteúdo KML não encontrado para este talhão.', 'error');
        }
    },
    
    handleKmlUpload() {
        const kmlFile = this.dom.kmlInput.files[0];
        const kmlName = this.dom.kmlNameInput.value.trim();

        if (!kmlFile) {
            this.showToast('Por favor, selecione um ficheiro KML.', 'error');
            return;
        }
        if (!kmlName) {
            this.showToast('Por favor, dê um nome ao novo talhão.', 'error');
            return;
        }
        ...
        reader.onload = async (event) => {
            ...
            this.setLoading(true, false, `A salvar "${kmlName}" no banco de dados...`);

            // CORRIGIDO: Usa o 'supabaseClient' para inserir os dados
            ...
            this.showToast(`"${kmlName}" salvo com sucesso!`, 'info');
            this.dom.kmlInput.value = '';
            this.dom.kmlNameInput.value = '';
            await this.loadKmlDatabase();
            this.setLoading(false);
        };

        reader.onerror = () => {
            this.showToast('Erro ao ler o ficheiro KML.', 'error');
        };
