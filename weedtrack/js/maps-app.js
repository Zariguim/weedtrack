...
const MapasApp = {
    dom: {
        ...
        kmlSelect: document.getElementById('kmlSelect'), // Adicionado
    ...
    state: {
        ...
        kmlDatabase: [], // Adicionado para armazenar os KMLs do CSV
    },
    ...
    init() {
        ...
        this.loadKmlDatabase(); // Adicionado para carregar o CSV
    ...
    bindEvents() {
        this.dom.kmlInput.addEventListener('change', e => {
            ...
            this.processKml(url);
            this.dom.kmlSelect.value = ""; // Limpa a seleção do dropdown
        });
        this.dom.kmlSelect.addEventListener('change', this.handleKmlSelect.bind(this)); // Adicionado
    ...
    // --- FUNÇÃO MODIFICADA ---
    loadKmlDatabase() {
        // Cole a URL pública da sua planilha aqui
        const googleSheetURL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQTwlR0N_Kp1skrRJfUTmyoceVFkgjCj3ub2GuJzoL_CjdQ7wtFz9iafkiBNcvXLbDZsj9ktY5ux3oi/pub?gid=1311924902&single=true&output=csv';
        ...
        Papa.parse(googleSheetURL, {
            ...
            skipEmptyLines: true,
            complete: (results) => {
                this.state.kmlDatabase = results.data;
                this.populateKmlSelect();
            },
            error: (err) => {
                console.error("Erro ao carregar KMLs do Google Sheets:", err);
                this.dom.kmlSelect.innerHTML = '<option value="">Erro ao carregar talhões</option>';
                this.showToast('Não foi possível carregar os talhões da planilha.', 'error');
            }
        });
    },

    // --- FUNÇÃO NOVA (sem alterações) ---
    populateKmlSelect() {
        this.dom.kmlSelect.innerHTML = '<option value="">Selecione na lista...</option>';
        this.state.kmlDatabase.forEach((talhao, index) => {
    ...
    handleKmlSelect(event) {
        ...
        const talhao = this.state.kmlDatabase[selectedIndex];
        if (talhao && talhao.conteudo_kml) {
            const kmlString = talhao.conteudo_kml;
            const blob = new Blob([kmlString], { type: 'application/vnd.google-earth.kml+xml' });
            const url = URL.createObjectURL(blob);
            this.processKml(url);
            this.dom.kmlInput.value = ""; // Limpa o input de arquivo
        } else {
