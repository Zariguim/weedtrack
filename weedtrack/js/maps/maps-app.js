...
const MapasApp = {
    dom: {
        ...
        csvSelect: document.getElementById('csvSelect'), // NOVO: Dropdown de CSV
    ...
    state: {
        ...
        kmlDatabase: [],
        csvRecords: [], // NOVO: Armazena os registros de CSV do Supabase
    },
    ...
    bindEvents() {
        ...
        // NOVO: Listener para o dropdown de CSV
        this.dom.csvSelect.addEventListener('change', () => {
            if (this.dom.csvSelect.value) {
                this.dom.csvInput.value = ''; // Limpa o input de ficheiro se um dropdown for selecionado
            }
        });
        // NOVO: Listener para o input de ficheiro para limpar o dropdown
        this.dom.csvInput.addEventListener('change', () => {
             if (this.dom.csvInput.files.length > 0) {
                this.dom.csvSelect.value = '';
            }
        });
    ...
    handleKmlSelect(event) {
        ...
        if (selectedId === "") {
            this.clearAllState();
            this.dom.csvSelect.innerHTML = '<option value="">Selecione um talhão primeiro...</option>';
            this.dom.csvSelect.disabled = true;
            return;
        };
        ...
        if (talhao && talhao.conteudo_kml) {
            ...
            this.processKml(url);
            this.loadCsvRecords(); // <<-- CHAMA A FUNÇÃO PARA CARREGAR OS CSVs
        } else {
            UIManager.showToast('Conteúdo KML não encontrado para este talhão.', 'error');
        }
    },

    // NOVA FUNÇÃO para carregar os CSVs do Supabase
    async loadCsvRecords() {
        this.dom.csvSelect.innerHTML = '<option value="">A carregar monitoramentos...</option>';
        this.dom.csvSelect.disabled = true;
        this.state.csvRecords = [];

        const selectedTalhaoId = this.dom.kmlSelect.value;
        if (!selectedTalhaoId) {
            this.dom.csvSelect.innerHTML = '<option value="">Selecione um talhão primeiro...</option>';
            return;
        }

        const talhaoInfo = this.state.kmlDatabase.find(t => t.id == selectedTalhaoId);
        if (!talhaoInfo) return;

        const fazendaName = talhaoInfo.Fazenda || 'Sem Fazenda';
        const talhaoName = talhaoInfo.nome_talhao;

        const { data, error } = await supabaseClient
            .from('registros_csv')
            .select('id, data, csv')
            .eq('fazenda', fazendaName)
            .eq('talhao', talhaoName)
            .eq('tag', 'monitoramento')
            .order('data', { ascending: false });

        if (error) {
            console.error("Erro ao buscar registros de CSV:", error);
            this.dom.csvSelect.innerHTML = '<option value="">Erro ao carregar</option>';
            return;
        }

        this.state.csvRecords = data;

        if (data.length === 0) {
            this.dom.csvSelect.innerHTML = '<option value="">Nenhum monitoramento encontrado</option>';
            return;
        }

        this.dom.csvSelect.innerHTML = '<option value="">Selecione um monitoramento salvo...</option>';
        data.forEach(record => {
            const option = document.createElement('option');
            option.value = record.id;
            const recordDate = new Date(record.data);
            option.textContent = recordDate.toLocaleString('pt-BR', {
                day: '2-digit', month: '2-digit', year: 'numeric',
                hour: '2-digit', minute: '2-digit'
            });
            this.dom.csvSelect.appendChild(option);
        });
        this.dom.csvSelect.disabled = false;
    },
    ...
    handleGenerateClick() {
        ...
        const selectedCsvRecordId = this.dom.csvSelect.value;
        const radiusM = parseFloat(this.dom.heatRadiusInput.value) || 50;

        if (!this.state.kmlGeoJSON) {
            UIManager.showToast('Por favor, selecione um talhão primeiro.', 'error');
            return;
        }

        if (csvFile) {
            this.processCsvFile(csvFile, radiusM);
        } else if (selectedCsvRecordId) {
            const record = this.state.csvRecords.find(r => r.id == selectedCsvRecordId);
            if (record && record.csv) {
                this.processCsvString(record.csv, radiusM);
            } else {
                UIManager.showToast('O registo de CSV selecionado é inválido.', 'error');
            }
        } else {
            UIManager.showToast('Por favor, selecione um monitoramento ou envie um ficheiro CSV.', 'error');
        }
    },
    
    // NOVA FUNÇÃO
    processCsvFile(csvFile, radiusM) {
        UIManager.setLoading(true, false, "A processar CSV do ficheiro...");
        Papa.parse(csvFile, {
            header: true, dynamicTyping: true, skipEmptyLines: true,
            complete: results => this.onCsvParseComplete(results, radiusM),
            error: err => {
    ...
    // NOVA FUNÇÃO
    processCsvString(csvString, radiusM) {
        UIManager.setLoading(true, false, "A processar CSV do banco de dados...");
    ...
    // NOVA FUNÇÃO (LÓGICA EXTRAÍDA)
    onCsvParseComplete(results, radiusM) {
        ...
        const customIcon = L.icon({
            iconUrl: 'Logo.svg', shadowUrl: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-shadow.png',
        ...
        data.forEach(row => {
            ...
            let label = pestsFound.length > 0
                ? `<b>Alvos:</b><br>${pestsFound.map(sp => `${sp}: ${row[sp]}`).join('<br>')}`
    ...
    setupActionButtons() {
        ...
        MapManager.reportControl.getContainer().innerHTML = ''; 
        MapManager.recomendacaoControl.getContainer().innerHTML = '';

        const zoneButton = document.createElement('button');
        zoneButton.className = 'w-48 bg-purple-600 text-white font-semibold py-2 px-4 rounded-md shadow-md hover:bg-purple-700 mt-2';
        ...
        reportButton.className = 'w-48 bg-blue-600 text-white font-semibold py-2 px-4 rounded-md shadow-md hover:bg-blue-700 mt-2';
        ...
        recomendacaoButton.className = 'w-48 bg-green-600 text-white font-semibold py-2 px-4 rounded-md shadow-md hover:bg-green-700 mt-2';
