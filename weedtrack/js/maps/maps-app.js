...
const MapasApp = {
    dom: {
        ...
        newTalhaoNameInput: document.getElementById('newTalhaoNameInput'),
    ...
    bindEvents() {
        ...
        // Delegação de eventos para a lista de talhões
        this.dom.talhoesListContainer.addEventListener('click', (e) => {
            ...
            if (action === 'edit') this.handleEditTalhao(id, name);
    ...
    async loadKmlDatabase() {
        UIManager.setLoading(true, false, "A carregar talhões do banco de dados...");
        
        const { data, error } = await supabaseClient
            .from('talhoes')
            .select('id, nome_talhao, conteudo_kml')
            .order('nome_talhao', { ascending: true });
        ...
        this.populateKmlSelect();
        this.populateTalhoesModal();
        UIManager.setLoading(false);
    },

    populateKmlSelect() {
        const currentVal = this.dom.kmlSelect.value;
        this.dom.kmlSelect.innerHTML = '<option value="">Selecione na lista...</option>';
        if (this.state.kmlDatabase && this.state.kmlDatabase.length > 0) {
            this.state.kmlDatabase.forEach((talhao) => {
                if (talhao.nome_talhao) {
            ...
            this.dom.kmlSelect.value = currentVal;
        } else {
            this.dom.kmlSelect.innerHTML = '<option value="">Nenhum talhão encontrado</option>';
        }
    },

    populateTalhoesModal() {
        const container = this.dom.talhoesListContainer;
        container.innerHTML = '';

        if (!this.state.kmlDatabase || this.state.kmlDatabase.length === 0) {
            container.innerHTML = '<p class="text-center text-slate-500 my-4">Nenhum talhão encontrado. Adicione um novo abaixo.</p>';
            return;
        }
        ...
        this.state.kmlDatabase.forEach(talhao => {
            if (talhao.nome_talhao) {
                ...
                talhaoEl.className = 'flex items-center justify-between p-3 rounded-lg hover:bg-slate-100 transition-colors duration-150';
                talhaoEl.innerHTML = `
                    <span class="font-medium text-slate-700">${talhao.nome_talhao}</span>
                    <div class="flex items-center gap-4">
                        <button data-action="edit" data-id="${talhao.id}" data-name="${talhao.nome_talhao}" class="text-slate-500 hover:text-blue-600 transition" title="Editar nome">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16"><path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zM13.75 3.5l-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/><path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/></svg>
                        </button>
    ...
    async handleKmlUpload() {
        const kmlFile = this.dom.newTalhaoFileInput.files[0];
        const kmlName = this.dom.newTalhaoNameInput.value.trim();

        if (!kmlFile) {
            UIManager.showToast('Por favor, selecione um ficheiro KML.', 'error');
            return;
        }
        if (!kmlName) {
            UIManager.showToast('Por favor, dê um nome ao novo talhão.', 'error');
            return;
        }
        ...
        reader.onload = async (event) => {
            ...
            UIManager.setLoading(true, false, `A salvar "${kmlName}" no banco de dados...`);

            const { error } = await supabaseClient.from('talhoes').insert([
                { nome_talhao: kmlName, conteudo_kml: kmlContent }
            ]);

            if (error) {
                console.error('Erro ao salvar KML:', error);
                UIManager.showToast(`Erro ao salvar: ${error.message}`, 'error');
            } else {
                UIManager.showToast(`"${kmlName}" salvo com sucesso!`, 'info');
                this.dom.newTalhaoFileInput.value = '';
                this.dom.newTalhaoNameInput.value = '';
                await this.loadKmlDatabase();
            }
            UIManager.setLoading(false);
        };
        reader.onerror = () => { UIManager.showToast('Erro ao ler o ficheiro KML.', 'error'); };
