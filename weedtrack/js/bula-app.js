// ===================================================================================
//  APLICATIVO WEEDTRACK BULA (LÓGICA DO MODAL)
// ===================================================================================
const BulaApp = {
    dom: {
    ...
    open(mapasState) {
        if (!mapasState.kmlGeoJSON) {
            MapasApp.showToast("Carregue um KML primeiro para criar uma recomendação.", "error");
            return;
        }
        this.state.parentState = mapasState;
        
        // 1. Resetar e preparar a interface da bula
        this.resetForm();
        
        // 2. Preencher área total a partir do KML
        const areaHa = turf.area(this.state.parentState.kmlGeoJSON) / 10000;
        this.dom.areaInput.value = areaHa.toFixed(2);
        
        // 3. Preencher a visualização de mapas de calor
        this.populateHeatmapPreview();

        // 4. Preencher o seletor de Zonas de Manejo
        this.populateZoneSelector();

        // 5. Atualizar os cálculos e abrir o modal
    ...
    async generatePdf() {
        MapasApp.setLoading(true, false, 'A gerar Bula...');
        ...
        if (!hasTotalRec && !hasZoneRec) {
            MapasApp.showToast("Nenhum defensivo foi adicionado.", "error");
            MapasApp.setLoading(false);
            return;
        }

        // --- 1. Capturar imagens do KML e Zonas de Manejo ---
        MapasApp.setLoading(true, false, 'A capturar imagens do mapa...');
        let kmlImage = null;
        let zoneImage = null;
        ...
        mapContainer.style.left = '-9999px'; // Esconde o container fora da tela
        ...
        await new Promise(r => setTimeout(r, 500)); // Espera inicial
        ...
        if (this.state.parentState.kmlGeoJSON) {
            ...
            await new Promise(r => setTimeout(r, 1000)); // Espera para os tiles carregarem
        ...
        // --- 2. Montar o PDF ---
        MapasApp.setLoading(true, false, 'A montar o relatório PDF...');
        ...
        // Adicionar páginas com os mapas de calor (3-column grid)
        if (this.state.parentState.generatedHeatmaps.length > 0) {
             MapasApp.setLoading(true, false, 'A anexar mapas...');
             doc.addPage();
    ...
    drawRecommendationBlock(doc, startY, title, areaInfo, pesticides, observations, image) {
        ...
        if (currentY > 250) { // Check for new page before starting
            doc.addPage();
            currentY = 20;
        }
        ...
        if (image) {
            ...
            const imgH = contentW * 0.75; // 4:3 aspect ratio
